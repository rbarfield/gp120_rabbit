...
NewData<-Fig3.data$Figure3_IgGA
...
Fig3.data$Figure3_ADCC %>% 
  ...
  ggplot(aes(x=Week,y=value,col=Vaccine,group=subject))+
  geom_point()+
  geom_line()+
...
#--- --- --- --- --- --- --- --- --- --- --- --- --- --- --- ---
      ...
      data.frame(Pvalues_nparld=c(signif(Resu1.titer.gp120$ANOVA.test.mod.Box[4],3),
        ...
        mutate(FDR_Pvalue=p.adjust(Pvalues_perm,"BH")) %>% 
...
Everything is significant. The permutation was done as the Titer analysis produced singular covariance matrix. We will use the permutation p-values (nparLD)
