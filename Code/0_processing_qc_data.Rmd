...
#--- --- --- --- --- --- --- --- --- --- --- --- --- --- --- ---
# Subset to the necssary info and derive proper names
#--- --- --- --- --- --- --- --- --- --- --- --- --- --- --- ---
...
#--- --- --- --- --- --- --- --- --- --- --- --- --- --- --- ---      
      ...
      Fig2.save.Bama<-Fig2.Bama %>% 
        ...
        mutate(Week=parse_number(str_remove_all(str_split_fixed(Description,"N",2)[,1],
                                                "wk")),
               RabbitID=str_split_fixed(Description,
                                        "[[:blank:]]",2)[,2]) %>% 
        select(Description,RabbitID,Week,everything()) %>% 
        arrange(Week)
...
Fig3.save.heat<-read_excel(file.path(pathtofile,"Figure3/",New.fig3))
...
#--- --- --- --- --- --- --- --- --- --- --- --- --- --- --- ---
    ...
    Fig3.corr.analysis<-Fig3.save.heat %>% 
...
Fig3.all<-list(Figure3_ADCC=Fig3.Save.ADCC,
               ...
               Figure3_heatmap=Fig3.corr.analysis)
...
This is the "binding avidity to gp120"
...
Fig4.save.spr<-read_excel(file.path(pathtofile,
                                    "Figure4/",New.fig4))
...
Fig4.actual_save.spr<-Fig4.save.spr %>% 
  pivot_longer(cols=c("kd","Avidity")) %>% 
  mutate(AtLowerBound=ifelse(value=="ND" & name=="kd",1,0)) %>% 
  mutate(AtUpperBound=ifelse(value=="ND" & name=="Avidity",
                             1,0)) %>% 
  mutate(Newvalue=ifelse(name=="kd" & value=="ND",0.1,
                         value)) %>% 
  mutate(Newvalue=ifelse(name=="Avidity" & value=="ND",
                         Response/0.1,Newvalue)) %>% 
  mutate(Newvalue=parse_number(Newvalue)) %>% 
  select(Sample,Week,name,Newvalue,AtLowerBound,AtUpperBound) %>% 
  bind_rows(Fig4.save.spr %>% 
...
New.fig5<-str_subset(list.files(file.path(pathtofile,
                                          "Figure5/")),
                     "Figure5 total IgG Fc wk0")
...
Fig5.IgC.wk0<-NewMean.and.CV %>%  rename(ID=newID) %>% 
...
## Figure 5 glycan gp120-specific wk14 



```{r Fig5_glycan_wk14, echo=T}
...
Fig5.IgC.glycan.wk14<-NewMean.and.CV %>% 
  rename(ID=newID) %>% 
  select(-N) %>% 
  mutate(What_Is="Individual") %>% 
  bind_rows(New.Conglom) %>% mutate(Week=14)
...
## Figure 5 glycan-function correlation
...
Our.sheets<-excel_sheets(file.path(pathtofile,"Figure5/",New.fig5))


Our.Data<-lapply(Our.sheets,function(x){
        read_excel(file.path(pathtofile,"Figure5/",New.fig5),
                   sheet=x) %>% 
    mutate(What=x)
  }
  )
...
Fig5.corr.analysis<-Our.Data %>% bind_rows() %>% 
...
Fig5.all<-list(Figure5_total_wk0=Fig5.IgC.wk0,
               Figure5_total_wk14=Fig5.IgC.wk14,
               Figure5_glycan=Fig5.IgC.glycan.wk14,
               Figure5_heatmap=Fig5.corr.analysis)


saveRDS(Fig5.all,
        file = file.path(pathtosave,"0_data_processed_fig5.RDS"))

```



# Figure 4 Mouse Additional information
...
Fig4.additional<-list(Figure4_FCR=Fig4_FcR,
                      Figure4_gp120=New.data,
                      Figure4_total=New.data_2)


saveRDS(Fig4.additional,
