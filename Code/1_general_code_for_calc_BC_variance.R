#------------------------------------------------------
#------------------------------------------------------
# Program: Code: 1_general_code_for_calc_BC_variance
#
# Author: Richard Barfield
#
# Creation date: 2022-02-03
# 
# Code to calculate BC sandwich variance as from 
# Mancl LA and DeRouen TA. A Covariance Estimator for GEE with Improved Small- Sample Properties. 
# Biometrics 2001;57: 126-134.
# 
# Code taken from geesmv package just slightly modified
#------------------------------------------------------
#------------------------------------------------------


MyGee.Var.MD<-function (formula, id, family = gaussian, data, corstr = "independence") 
{
    if (is.null(data$id)) {
    ...
    return(list(beta_est=beta_est,cov.beta = (cov.beta), cov.var = cov.var))
}
